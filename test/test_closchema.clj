...
(describe validate "items"
  ...
  (describe validate "with array of schemas"      
    ...
    (describe "with unique items equal true"      
      :given [s (ctx {:type "array" :uniqueItems true})]
      
      (describe "should allow array with same items"
        (it "of type numbers" (validate s [1 1])) 
        (it "of type strings" (validate s ["a" "a" "a"]))
        (it "of type objects" (validate s [{:a 1} {:a 1} {:a 1}])))
 
      (describe "should not allow different items"
